...
import type { SecretHuntLogEntry } from "@/lib/secret-hunter/types";
...
interface HistoryPanelProps {
  entries: SecretHuntLogEntry[];
  activeId?: string;
  onSelect: (entry: SecretHuntLogEntry) => void;
  onClear: () => void;
}
        ...
        {entries.map((entry) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              ...
              <span className={cn("text-xs font-mono font-bold", RISK_TEXT[entry.riskLevel])}>
                {entry.riskLevel} · {entry.findingsCount}
              </span>
            </div>
